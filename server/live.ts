...
const SYSTEM_INSTRUCTION = [
  ...
  '1) (손님이 답했으면) 지금 단계를 2~4문장으로 읽어준다. 그 단계에 적힌 궁 가운데',
  '   최소 두 개를 이름과 함께 짚는다.',
  ...
  '',
  '[반복 금지 — 매우 중요]',
  '이미 읽은 단계는 두 번 읽지 않는다. 방금 한 말을 다시 하지 않는다.',
  ...
  '이때부터는 4단계 스캔을 처음부터 다시 시작하지 않는다. 손님이 묻는 것에만 답한다.',
  '답은 3~4문장. 화면에 보이는 얼굴 특징을 근거로 삼되, 앞에서 이미 한 말을 그대로',
...
const NUDGE_TEXT =
  ...
  '볼 수 없으면 무엇이 어떻게 가렸는지 짚어 한 번 더 청하고, 볼 수 있으면 지금 보기로 한 ' +
  '부위를 읽어준 뒤 다음 부위로 넘어가세요. 부위를 건너뛰지는 마세요.)'
